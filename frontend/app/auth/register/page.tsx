...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('register.errorFallback', 'فشل إنشاء الحساب'));
    }
  };
        ...
        >
          <div className="w-full max-w-md">
            ...
            <motion.h1 variants={item} className="auth-headline text-[44px] leading-[1.05] tracking-[-0.02em] mb-4">
              {t('register.headlineLead', 'Build')}{' '}
              <em className="auth-headline-em">{t('register.headlineEmphasis', 'your shelf.')}</em>
            </motion.h1>

            <motion.p variants={item} className="auth-subhead text-[15px] leading-relaxed mb-9">
              {t(
                'register.subhead',
                'An account opens the library to you in under a minute. Notes, highlights, and citations live with you from the first page.'
              )}
            </motion.p>
            ...
            <motion.div variants={item} className="auth-divider flex items-center gap-4 mb-7">
              ...
              <span className="auth-divider-label text-[11px] tracking-[0.16em] uppercase">
                {t('register.orEmail', 'Or with email')}
              </span>
              <span className="auth-divider-line flex-1 h-px" />
            </motion.div>
            ...
            <form onSubmit={handleSubmit}>
              <motion.div variants={item} className="grid grid-cols-2 gap-3 mb-4">
                <div>
                  <label htmlFor="firstName" className="auth-field-label block text-[12px] tracking-[0.08em] uppercase mb-2">
                    {t('register.firstName', 'الاسم الأول')}
                  </label>
                  <Input
                    id="firstName"
                    type="text"
                    ...
                    placeholder={t('register.placeholder.firstName', 'محمد')}
                    disabled={isLoading}
                    autoComplete="given-name"
                  />
                </div>
                <div>
                  <label htmlFor="lastName" className="auth-field-label block text-[12px] tracking-[0.08em] uppercase mb-2">
                    {t('register.lastName', 'اسم العائلة')}
                  </label>
                  <Input
                    id="lastName"
                    type="text"
                    ...
                    placeholder={t('register.placeholder.lastName', 'الأنصاري')}
                    disabled={isLoading}
              ...
              <motion.div variants={item} className="mb-7">
                <label htmlFor="password" className="auth-field-label block text-[12px] tracking-[0.08em] uppercase mb-2">
                ...
                <p className="auth-field-hint mt-2 text-[12px]">
                  {t('register.minPassword', 'يجب أن تكون كلمة المرور 8 أحرف على الأقل.')}
                </p>
              </motion.div>
              ...
              <motion.div variants={item}>
                ...
                >
                  {isLoading ? (
                    <>
                      <Spinner /> {t('register.creating', 'جارٍ إنشاء الحساب...')}
                    </>
                  ) : (
                    <>
                      {t('register.create', 'إنشاء حساب')}
