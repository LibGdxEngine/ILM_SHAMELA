...
| `backfill_word_geometry.py` | Backfill per-word OCR bounding boxes (`layout.blocks[].words`) for PDF-overlay documents via the tesseract sidecar's `/words` endpoint + `extract_word_geometry_task`. **Usage**: `python manage.py backfill_word_geometry [--document-id N ...| --all] [--pages 1-20,25] [--force] [--sync] [--dry-run] [--stagger S] [--batch-pages N]`. No args lists layout docs with geometry status. Idempotent: pages whose blocks already carry `word_geometry` are skipped unless `--force`. `--sync` runs inline (useful for a few pages); async enqueues one Celery task per document (or per `--batch-pages` slice). |
