...
| `test_tasks.py` | Async document processing task (`process_document_task`). Tests file parsing, OCR fallback selection, language detection, content splitting into pages, thumbnail generation, DocumentChunk creation with embeddings, ES indexing, error tracking, and the `.md` branch (asserts Tika is never called, pages come from `{N}` markers, `ocr_engine_used='markdown'`). Mocks `build_embedding()` and `DocumentIndex`. |
...
| `test_serializer_validation.py` | File upload validation on `DocumentSerializer`. Tests extension whitelist (.pdf, .doc, .docx, .txt, .md), rejects unsupported types (.exe, etc.) and an `ocr_layout` attached to a non-PDF. Verifies size limits (25 MB documents, 10 MB covers). |
| `test_markdown_processing.py` | The marker/datalab Markdown page splitter (`_split_markdown_pages`). Pure `SimpleTestCase` — no DB, no mocks. Covers `{N}-----` separator detection, marker lines dropped from output, positional (not declared) page numbering, blank/leading/trailing separators, CRLF, and non-separators (bare `---`, mid-line `{1}---`). |
